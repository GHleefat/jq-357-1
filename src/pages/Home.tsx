...
import { Mic, Square, RotateCcw, Save, Sparkles } from 'lucide-react';
...
export default function Home() {
  ...
  const { isRecording, recordingTime, audioLevel, startRecording, stopRecording, error } =
      ...
      <div className="relative w-full h-40 bg-white/60 rounded-3xl border border-stone-200/60 shadow-sm overflow-hidden p-4">
        ...
        {isRecording && (
          <div className="absolute top-4 right-4 flex items-center gap-2">
            ...
            <span className="text-sm font-mono text-stone-600">{formatTime(recordingTime)}</span>
          </div>
        )}
      </div>
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <span className="text-4xl">{EMOTION_CONFIGS[emotionResult.emotion].emoji}</span>
                <div>
                  <p className="text-sm opacity-70" style={{ color: EMOTION_CONFIGS[emotionResult.emotion].color }}>
                    检测到情绪
                  </p>
                  <h3
                    className="text-2xl font-bold"
                    style={{ color: EMOTION_CONFIGS[emotionResult.emotion].color, fontFamily: '"Lora", Georgia, serif' }}
                  >
                    {EMOTION_CONFIGS[emotionResult.emotion].label}
                  </h3>
                </div>
              </div>
              <EmotionBadge emotion={emotionResult.emotion} score={emotionResult.score} size="lg" />
            </div>

            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span style={{ color: EMOTION_CONFIGS[emotionResult.emotion].color }}>置信度</span>
                <span className="font-mono" style={{ color: EMOTION_CONFIGS[emotionResult.emotion].color }}>
              ...
              <div className="w-full h-2 bg-white/50 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: EMOTION_CONFIGS[emotionResult.emotion].color,
                  }}
                />
              </div>
            </div>
